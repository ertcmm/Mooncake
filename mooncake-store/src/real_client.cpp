...
RealClient::batch_get_buffer_internal(
    ...
    auto query_results = client_->BatchQuery(keys);    // 2. Prepare for batch get: filter valid keys and prepare buffers
    struct KeyOp {
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        const auto &res = client_->GetPreferredReplica(query_result_values.replicas);
        if (!res) {
    ...
    // 3. Execute batch get for memory/disk
    if (!valid_ops.empty()) {
    ...
    // 4. Execute batch get for local disk
